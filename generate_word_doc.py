...
def fix_hebrew_punctuation(text):
    """
    מתקן רווחים סביב סימני פיסוק בעברית ומירכאות
    """
    import re

    # תיקון מירכאות חכמות למירכאות רגילות
    ...
    text = re.sub(r'"\s*([א-ת])', r'" \1', text)  # רווח אחרי מירכאה פותחת
    text = re.sub(r'([א-ת])\s*"', r'\1 "', text)  # רווח לפני מירכאה סוגרת

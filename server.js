...
function applyParagraphBreaking(text) {
  ...
  text = text
    ...
    // תיקון מילים עם ר' (רב)
    .replace(/ר\s*'\s*([א-ת])/g, 'ר\' $1')      // ר' משה -> ר' משה
    ...
    // תיקון מילים שנצמדו (אין רווח ביניהן)
    .replace(/([א-ת]{3,})([א-ת]{3,})/g, (match, word1, word2) => {
      // רק אם זה לא קיצור או מילה מוכרת
      const combined = word1 + word2;
      if (combined.match(/^(יודעתראו|אומרתאני|רוצהאני|אמרתי|אמרלי|שאלתי|ענהלי)$/)) {
        return word1 + ' ' + word2;
      }

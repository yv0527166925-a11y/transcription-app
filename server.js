...
async function transcribeAudioChunkWithFlashFallback(chunkPath, chunkIndex, totalChunks, filename, language, customInstructions, retryCount = 0) {
  ...
  try {
    ...
    // Second attempt: Fallback to Gemini Flash Latest
    try {
      ...
      console.log(`✅ Gemini Flash Latest fallback successful for chunk ${chunkIndex + 1} (${transcription.length} chars)`);
      return transcription;
    } catch (error2) {
      console.log(`⚠️ Gemini Flash Latest fallback also failed for chunk ${chunkIndex + 1}:`, error2.message);
...
תחזיר את הטקסט המחולק לפסקאות עם \\n\\n בין כל פסקה:`;
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      ...
      try {
        ...
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Smart division timeout after 1.5 minutes')), 90 * 1000)
        );
    ...
    // If we get here, all fast attempts failed
...
תחזיר את הטקסט המחולק לפסקאות עם \\n\\n בין כל פסקה:`;
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          ...
          console.log(`⏳ Waiting 20 seconds before next Flash Latest retry...`);
...
תחזיר את הטקסט המחולק לפסקאות עם \\n\\n בין כל פסקה:`;
      ...
      for (let attempt = 1; attempt <= 2; attempt++) {
        try {
          ...
          console.log(`⏳ Waiting 20 seconds before fallback retry...`);
          await new Promise(resolve => setTimeout(resolve, 20000));
        }
      }
...
async function smartParagraphDivisionChunked(text, maxChars, language = null) {
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      if (i > 0) {
        console.log('⏱️ Waiting 2 seconds between chunks...');
        ...
        while (retryCount <= maxRetries && !chunkTranscription) {
          try {
            if (retryCount > 0) {
              ...
              const backoffDelay = backoffDelays[retryCount - 1] || 10000;

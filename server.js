  ...
  try {
    ...
    sections.forEach(section => {
      ...
      combinedSection = combinedSection
        ...
        .replace(/([א-ת])\s*"\s*([א-ת])/g, '$1"$2')           // תקן אכרונים כמו רמב"ם, הארי"ז, ז"ל
        .replace(/([א-ת])\s*'\s*([א-ת])/g, '$1\'$2')          // תקן אכרונים כמו רש"י
        // אחר כך תקן פיסוק בין מילים נפרדות - כולל צמודות וכולל עם רווחים
        .replace(/([\u0590-\u05FF])\.(\s*)([\u0590-\u05FF\[])/g, '$1.&nbsp;$3')      // תקן נקודות בין מילים עבריות (כולל ניקוד)
        .replace(/([\u0590-\u05FF]),(\s*)([\u0590-\u05FF\"])/g, '$1,&nbsp;$3')  // תקן פסיקים צמודים למילים (כולל ניקוד)
        .replace(/([\u0590-\u05FF])"\s+([\u0590-\u05FF])/g, '$1"&nbsp;$2')     // תקן גרשיים בין מילים נפרדות (רק עם רווח)

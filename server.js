  ...
  try {
    ...
    sections.forEach(section => {
      ...
      combinedSection = combinedSection
        // קודם תקן אכרונים עבריים - גרשיים בתוך מילה אחת
        .replace(/([א-ת]{2,})\s*"\s*([א-ת]{1,2})/g, '$1"$2')     // תקן אכרונים כמו רמב"ם, תנ"ך
        .replace(/([א-ת]{2,})\s*'\s*([א-ת]{1,2})/g, '$1\'$2')    // תקן אכרונים כמו רש"י
        // אחר כך תקן פיסוק בין מילים נפרדות - כולל צמודות וכולל עם רווחים
        .replace(/([א-ת])\.(\s*)([א-ת\[])/g, '$1.&nbsp;$3')      // תקן נקודות בין מילים עבריות (כולל סוגריים)
        .replace(/([א-ת]),(\s*)([א-ת\"])/g, '$1,&nbsp;$3')  // תקן פסיקים צמודים למילים (כולל גרשיים)
        .replace(/([א-ת])"(\s*)([א-ת])/g, '$1"&nbsp;$3')     // תקן גרשיים צמודים בין מילים עבריות
        .replace(/\](\s*)([א-ת\"])/g, ']&nbsp;$2')           // תקן סוגריים מרובעים צמודים למילים
        .replace(/\",\"(\s*)([א-ת])/g, '",&nbsp;\"$2')       // תקן גרשיים+פסיק+גרשיים צמודים
        .replace(/([א-ת])'\s+([א-ת])/g, '$1\'&nbsp;$2')    // תקן גרש בין מילים נפרדות (רק עם רווח)
        .replace(/([א-ת])\?(\s*)([א-ת])/g, '$1?&nbsp;$3')    // תקן סימני שאלה בין מילים עבריות
        .replace(/([א-ת])!(\s*)([א-ת])/g, '$1!&nbsp;$3')     // תקן סימני קריאה בין מילים עבריות
        .replace(/([א-ת]):(\s*)([א-ת])/g, '$1:&nbsp;$3')     // תקן נקודתיים בין מילים עבריות
        .replace(/([א-ת]);(\s*)([א-ת])/g, '$1;&nbsp;$3')     // תקן נקודה פסיק בין מילים עבריות
        // ניקוי כללי
